...
const BeeKeeperModal: React.FC<BeeKeeperModalProps> = ({
  mode,
  beekeeper,
  isModalOpen,
  handleClose,
  ...
  const [hasLastnameError, setHasLastnameError] =
    React.useState<boolean>(false);
  ...
  const deleteBeekeeper = async () => {
    if (mode === BeeKeeperModalMode.Edition) {
      ...
      await UserApiClient.deleteUserById(
        { id: id?.toString() || "-1" },
        {
          headers: new Headers([["Token", loggedUser?.token || ""]]),
        }
    ...
    <>
      ...
      <Dialog open={isModalOpen} onClose={handleClose}>
        ...
        >
          <p style={{ margin: 0, padding: 0 }}>
            {mode === BeeKeeperModalMode.Edition
              ? `${beekeeper?.informations?.firstname} ${beekeeper?.informations?.lastname}`
              : "Nouvel apiculteur"}
          </p>
          {mode === BeeKeeperModalMode.Edition && (
            <IconButton
          ...
          <div style={{ display: "flex", flexDirection: "row", padding: 20 }}>
            ...
            >
              ...
              <TextField
                ...
                label={
                  hasPasswordError ? "Mot de passe requis" : "Mot de passe"
                }
                style={{
                  marginBottom: 16,
                  marginTop: 16,
        ...
        <DialogActions>
          ...
          >
            {mode === BeeKeeperModalMode.Edition ? "Modifier" : "Ajouter"}
          </LoadingButton>
        </DialogActions>
      </Dialog>
    </>
