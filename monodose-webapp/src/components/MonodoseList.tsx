...
const columns: readonly Column[] = [
  ...
  {
    id: "dates.endofproduction",
    label: "Date fin de production",
    minWidth: 170,
    format: (m) => m.dates?.endOfProduction,
  },
  {
    id: "dates.startofproduction",
    label: "Date début de production",
    minWidth: 170,
    format: (m) => m.dates?.startOfProduction,
  },
  {
    id: "dates.dluo",
    label: "Date DLUO",
    minWidth: 170,
    format: (m) => m.dates?.dluo,
  },
  {
    id: "honeyvariety",
    label: "Variété du miel",
    minWidth: 170,
    format: (m) => m.honeyVariety,
  },
];

const MonodoseList: React.FC = () => {
  const [isModalOpen, setIsModalOpen] = React.useState<boolean>(false);
  const [isQrCodeModalOpen, setIsQrCodeModalOpen] =
    React.useState<boolean>(false);
  const [monodoseID, setMonodoseID] = React.useState<number>(0);
  const [selectedMonodose, setSelectedMonodose] = React.useState<Monodose>();
  const [modalMode, setModalMode] = React.useState<MonodoseModalMode>(
    MonodoseModalMode.Edition
  );
  ...
  const handleCloseCreation = (monodoseIDCreated: number) => {
    setIsModalOpen(false);
    if (monodoseIDCreated > 0) {
      setMonodoseID(monodoseIDCreated);
  ...
  return (
    <>
          ...
          <Table stickyHeader aria-label="sticky table">
            ...
            <TableBody>
              {rows?.map((row, index) => (
                <TableRow
                  hover
                  sx={{ "&:focus": { backgroundColor: "rgba(0, 0, 0, 0.1)" } }}
                  role="checkbox"
                  tabIndex={0}
                  key={index}
                  onClick={() => {
      ...
      <MonodoseModal
        monodose={selectedMonodose}
        mode={modalMode}
        isModalOpen={isModalOpen}
        handleClose={handleCloseCreation}
      />
      <QrCodeModal
        monodoseID={monodoseID}
