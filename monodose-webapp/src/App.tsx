...
const App: React.FC = () => {
  const [loggedUser, setLoggedUser] = React.useState<Logged | undefined>(
    undefined
  );
  ...
  const checkAuthentification = (component: JSX.Element): JSX.Element => {
    return loggedUser?.user === undefined ? (
      <Login
        onLogin={(newLoggedUser: Logged) => setLoggedUser(newLoggedUser)}
      />
    ) : (
      component
    );
    ...
    <UserContext.Provider value={value}>
      <Routes>
        ...
        <Route
          ...
          element={checkAuthentification(<AdminPage />)}
        />
        <Route
          path={AppRoutes.Form}
          element={checkAuthentification(<FormPage />)}
        />
      </Routes>
